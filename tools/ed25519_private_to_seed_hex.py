#!/usr/bin/env python3
"""
Extract the raw Ed25519 private seed (64 lowercase hex = 32 bytes) from a private key file.

Use the same value written to bootstrap_key/ed25519_private.hex by bootstrap_key_provision.py.
...
Requires: cryptography (see tools/requirements-ed25519-tools.txt).
  ...
  python3 tools/ed25519_private_to_seed_hex.py ~/.ssh/mpc_auth_ed25519
  python3 tools/ed25519_private_to_seed_hex.py key.pem --prompt
"""
...
def private_key_to_seed_hex(data: bytes, password: bytes | None) -> str:
...
def main() -> None:
    ...
    try:
        print(private_key_to_seed_hex(data, password=password))
    except ValueError as e:
