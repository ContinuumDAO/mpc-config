...
| **`agent-intent-rules.yaml`** | Free-text intent → pack boost + hints + optional **`always`** turn hints + **`loadMcpServers`** auto-load (host YAML; never short-circuits the LLM). Protocol and third-party MCP policy lives here, not in mpc-auth. APIs: **`GET /getAgentIntentRulesConfig`**, **`POST /upsertAgentIntentRulesConfig`**, **`POST /resetAgentIntentRulesFromDefaults`**. |
