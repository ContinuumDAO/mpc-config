...
| **`agent-intent-rules.yaml`** | Free-text intent → pack boost + hints + optional **`always`** turn hints + **`loadMcpServers`** auto-load (host YAML; never short-circuits the LLM). Protocol and third-party MCP policy lives here, not in mpc-auth. APIs: **`GET /getHostYamlConfig?kind=agent-intent-rules`**, **`POST /upsertHostYamlConfig`**, **`POST /resetHostYamlFromDefaults`**. |
| **`continuum-dao-vote-policy.yaml`** | ContinuumDAO vote defaults (`trustedProposers`, `defaultAction`, type/signature/target/scam rules). Injected when the **`continuum-dao-vote-policy`** skill loads. APIs: **`GET /getHostYamlConfig?kind=continuum-dao-vote-policy`**, **`POST /upsertHostYamlConfig`**, **`POST /resetHostYamlFromDefaults`**. |
