...
| *(no `MCP_servers.json` here)* | Optional MCP catalog is **`agent_llm_config.defaults/MCP_servers.json`** on the host mount only — activate via **`POST /addMcpServerFromCatalog`**. Custom/active servers: **`POST /addMcpServer`** → MongoDB. Catalog rows may set **`kind`** (`search-engine` / `research-data`) and **`searchTags`** so the host classifies and keyword-searches new servers after `git pull` — no mpc-auth rebuild. |
| **`trade-desk.yaml`** | Trade prefill defaults (host YAML). APIs: **`GET /getHostYamlConfig?kind=trade-desk`**, **`POST /upsertHostYamlConfig`**, **`POST /resetHostYamlFromDefaults`**. |
| **`orchestration-plan.yaml`** | Plan modes + execution policy + **`followOn`** (shared follow-on recipe: inherit prior mode, store host trade ideas). APIs: **`GET /getHostYamlConfig?kind=orchestration-plan`**, **`POST /upsertHostYamlConfig`**, **`POST /resetHostYamlFromDefaults`**. After the mpc-auth loader upgrade, change leaves/matchers/budgets/verify text here — no binary rebuild. |
| **`agent-intent-rules.yaml`** | Free-text intent → pack boost + hints + optional **`always`** turn hints + **`loadMcpServers`** auto-load + optional **`verify`** (`interview` / `lookup` / `deliverable`) (host YAML; never short-circuits the LLM). Protocol and third-party MCP policy lives here, not in mpc-auth. APIs: **`GET /getHostYamlConfig?kind=agent-intent-rules`**, **`POST /upsertHostYamlConfig`**, **`POST /resetHostYamlFromDefaults`**. |
| **`continuum-dao-vote-policy.yaml`** | ContinuumDAO vote defaults (`trustedProposers`, `defaultAction`, type/signature/target/scam rules). Committee addresses come from the Constitution, not this file. Injected when the **`continuum-dao-vote-policy`** skill loads. APIs: **`GET /getHostYamlConfig?kind=continuum-dao-vote-policy`**, **`POST /upsertHostYamlConfig`**, **`POST /resetHostYamlFromDefaults`**. |
| **`cron/trade-cron.yaml`** | Node-wide **`tradeConsensus`** / **`tradeBuild`** defaults for scheduled trade-analysis cron runs (host YAML). APIs: **`GET /getHostYamlConfig?kind=cron-trade`**, **`POST /upsertHostYamlConfig`**, **`POST /resetHostYamlFromDefaults`**. |
