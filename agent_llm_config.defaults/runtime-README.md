...
| **`orchestration-plan.yaml`** | Plan modes + execution policy (host YAML). APIs: **`GET /getHostYamlConfig?kind=orchestration-plan`**, **`POST /upsertHostYamlConfig`**, **`POST /resetHostYamlFromDefaults`**. After the mpc-auth loader upgrade, change leaves/matchers/budgets/verify text here — no binary rebuild. |
