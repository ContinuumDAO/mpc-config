...
| **`agent-intent-rules.yaml`** | Free-text intent → pack boost + hints + optional **`always`** turn hints + **`loadMcpServers`** auto-load + optional **`verify`** (`interview` / `deliverable`) (host YAML; never short-circuits the LLM). Protocol and third-party MCP policy lives here, not in mpc-auth. APIs: **`GET /getHostYamlConfig?kind=agent-intent-rules`**, **`POST /upsertHostYamlConfig`**, **`POST /resetHostYamlFromDefaults`**. |
