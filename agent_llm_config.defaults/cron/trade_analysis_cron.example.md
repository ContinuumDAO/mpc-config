...
| **`uniswap`** | `load_defi_protocol` **uniswap** (V4) | **Separate source required** — Uniswap has no protocol OHLCV. Cron message must name one: load **hyperliquid** or **gmx** for perp candles, **or** load **coingecko** / **coinmarketcap-public** for spot series (trend/levels/patterns need **`dataKind: ohlcv`** — prefer HL/GMX for full menu). | `sizeUsdHuman` (spot swap USD) |
