...
- Each **`analyze_*`** step upserts a typed setup into **`conversation.tradeIdeas[]`** on the **`[Cron]`** thread (e.g. **`analyze_trend_structure`** → `trend_structure`, **`analyze_key_levels`** → `key_levels`, **`analyze_key_level_fibonacci`** → `key_level_fibonacci`, **`analyze_elliott_waves`** → `elliott_waves` — separate ideas from the same OHLCV session).
- Optional fenced **`tradeConsensus`** YAML in the job **`message`** gates multi-analysis agreement (node injects a matrix hint). Valid `requiredSources` include `trend_structure`, `key_levels`, `key_level_fibonacci`, `elliott_waves`, `chart_pattern`, `momentum`, `bollinger_bands`, `moving_averages`, …
