...
3. **On-chain effects vs signaling**
   ...
   - Real effects → collect each action: **network** (`name` / `label` / `chainId` from `GET /protocol/networks`; Linea is home-chain even when `c3governor` is set), **target**, **value** (wei string), **signature**, **inputs** (`name`, `type`, `value`). Flag C3 only when `network.name` is not Linea; also flag ETH value, `approve` / `transfer` / owner / upgrade as risks in the preview.
