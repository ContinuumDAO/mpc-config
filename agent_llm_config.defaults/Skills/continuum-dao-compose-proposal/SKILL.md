...
Load when the operator wants to **create** a proposal in chat (“draft a proposal”, “propose that we…”, “put this on-chain”). Always load **`continuum-dao-proposal-standards`** and **`execution-policy`** before any multi-sign submit. Load **`continuum-dao-proposals`** only if they also want a live-list briefing. Help the operator draft to the official Proposal Format.
...
7. **Vision / Mission** — fetch the Constitution. If the draft does not further them, **tell the operator** and recommend a rewrite. If they **insist**, continue and append the red/amber appendix from that skill.
8. **Forum thread is required and must be created first.** Do not propose with the homepage or an invented URL. `forumKey` must be `/topic/:tid` or `/t/:tid` on forum.continuumdao.org.

## Forum login / logout / write

Reads do not need a ticket. Writes (`forum_create_topic`, `forum_reply`, `forum_react`) need a ticket from KeyGen **EIP-712** sign-in (no EVM tx).
...
3. `continuum__ctm_continuum_dao_forum_me` to confirm session and whether they can create a proposal thread (`getVotes >= proposalThreshold()` on the forum).
4. `continuum__ctm_continuum_dao_forum_sections` then **`continuum__ctm_continuum_dao_forum_create_topic`**. Section: Constitution → `constitution`, Treasury → `treasury`, otherwise `ideas`. Title/body must be English. Body = formatted proposal (≤ 6200) **plus** the standards appendix if the operator insisted after a failed check. Keep the returned **`url`**.
...
Never call `build_propose_*` or `register_proposal` until `forum_create_topic` has returned a topic URL.
...
1. Forum sign-in (if needed) → `forum_create_topic` → keep `url` as `forumKey`.
