...
| **`agent-intent-rules.yaml`** | Same name | Free-text intent → pack boost, **`always`** turn hints, **`loadMcpServers`**, optional **`verify`** (`interview` / `deliverable`), and vendor/protocol playbooks (never short-circuits the LLM). Host-parsed YAML; mpc-auth stays vendor-agnostic. |
