...
**Peer egress (`wg-egress`):** **`GET /vpn/egress/status`** on **ManagementAPIsPort** (**8080**) — localhost only; read-only for the local node app. **`GET /vpn/egress/availableExits`** discovers peer exits via **MQTT** (`VpnEgressStatusRequest` / `VpnEgressStatusReply` on the group relay), using **`KeyList`** pubkeys from **`configs.yaml`** — not HTTP to peer **8080** or PublicDiscoveryPort. **Client config issuance** uses the **MQTT relay** (existing **8883** broker path): consumer **`POST /vpn/egress/requestClientConfig`** on **:8080** (localhost) → **`VpnEgressIssueRequest`** → egress → **`VpnEgressIssueReply`**. **`POST /vpn/egress/issueClientConfig`** remains on management **:8080** (loopback / signed debug only). **`WireGuardEgress.EndpointHost`** in **`configs.yaml`** (auto-set by **`process_config.sh`**) supplies the WireGuard **`Endpoint`** in issued client configs.
...
2a. **Host libexec + systemd units** (unless **`MPC_AUTH_SKIP_SYSTEMD_SYNC=1`** or **`PROCESS_CONFIG_SKIP_SYSTEMD=1`**): when mpc-auth systemd units are already under **`/etc/systemd/system/`**, runs **`${MPC_AUTH_COMPOSE_WORKDIR}/systemd/install-mpc-auth-docker-systemd.sh --no-env`** so **`/usr/local/libexec/mpc-auth/*.sh`** and unit files match the pulled repo (**`/etc/default/mpc-auth-docker`** is preserved). Failures are warnings only — Docker pull/restart continues.
...
9. **Companion continuumdao-node-app:** when **`MPC_AUTH_UPDATE_NODE_APP=1`** (default) and **`NODE_APP_IMAGE`** is set, looks up Docker Hub’s newest **`vMAJOR.MINOR.PATCH`** and pulls that tag. **`NODE_APP_TAG`** is not the pull target (a stale pin is ignored). If Hub cannot be reached, it falls back to **`NODE_APP_TAG`**. The pulled tag is pinned with a one-shot compose override and **`dashboard`** is recreated with **`--pull never`**, so Compose cannot keep the previous image just because the compose file still says **`:latest`**. If the container is still not that image, it is removed and created again. If it still does not start on the pulled image, the previous image is tagged back and started, and the status says the node app did not update.
10. **Companion MCP server:** same as the node app, for **`MCP_SERVER_IMAGE`** / **`continuum-mcp`**. **`MCP_SERVER_TAG`** from **`configs.yaml`** **`ContinuumMcpServer.Tag`** (synced into **`/etc/default/mpc-auth-docker`** by **`process_config.sh`**) is ignored when Hub answers. A deleted pin such as **`v1.3.14`** is not pulled.
11. **Prune old images:** after a successful full update (not **`restartOnly`**), removes unused images for **mpc-auth** and the companion repos — superseded version tags (e.g. **`v1.2.6`**) and dangling **`<none>`** layers from same-tag digest bumps — while keeping the verified/pulled refs, compose **`:latest`**, and any image still referenced by a container. If the new IMAGE ID did not change, prune leaves the running image alone. Set **`MPC_AUTH_PRUNE_OLD_IMAGES=0`** in **`/etc/default/mpc-auth-docker`** to disable.
...
**`mpc-auth-docker-update.sh`** (Maintenance **`POST /updateMpcAuth`** and **`restartOnly`**) runs **`install-mpc-auth-docker-systemd.sh --no-env`** after **`git pull`** when mpc-auth systemd units are already installed — same sync as **`process_config.sh`**, without a separate manual step.

**`./process_config.sh`** (without **`--no-systemd`**) also runs **`install-mpc-auth-docker-systemd.sh --no-env`** automatically when **`mpc-auth-docker-restart.service`**, **`mpc-auth-docker-pending-update.service`**, or **`mpc-auth-docker-pending-reboot.service`** is already installed — so **`git pull` → `process_config.sh`** refreshes libexec and unit files (**`/etc/default/mpc-auth-docker`** is preserved).
