...
10. **Companion MCP server:** when **`MPC_AUTH_UPDATE_MCP_SERVER=1`** (default) and **`MCP_SERVER_IMAGE`** is set: if **`MCP_SERVER_TAG`** is **`latest`** (the configs.yaml default), the script looks up Docker Hub’s newest **`vMAJOR.MINOR.PATCH`**, pulls that tag (and **`latest`**), then **`docker tag`**s it onto **`${MCP_SERVER_IMAGE}:latest`** (or **`MCP_SERVER_COMPOSE_IMAGE_REF`**) so compose’s **`:latest`** is the new layers — same retag as mpc-auth. Then **`docker compose pull --policy always`** and **`up -d --no-deps --force-recreate`** for **`continuum-mcp`**. A stale local **`latest`** alias is why Maintenance can show Hub **`v1.3.86`** while **`docker images`** still shows the previous IMAGE ID. **`process_config.sh`** syncs **`ContinuumdaoNodeApp`** and **`ContinuumMcpServer`** from **`configs.yaml`** into **`/etc/default/mpc-auth-docker`**.
