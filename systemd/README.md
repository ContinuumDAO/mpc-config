...
9. **Companion continuumdao-node-app:** when **`MPC_AUTH_UPDATE_NODE_APP=1`** (default) and **`NODE_APP_IMAGE`** is set, looks up Docker Hub’s newest **`vMAJOR.MINOR.PATCH`** and pulls that tag. **`NODE_APP_TAG`** is not the pull target (a stale pin is ignored). If Hub cannot be reached, it falls back to **`NODE_APP_TAG`**. The pulled image is **`docker tag`**ged onto the compose service image, then **`docker compose up -d --no-deps --force-recreate --pull never`** recreates **`dashboard`**.
