...
9. **Companion dashboard:** when **`MPC_AUTH_UPDATE_NODE_APP=1`** (default) and **`NODE_APP_IMAGE`** is set, runs **`docker pull $NODE_APP_IMAGE:$NODE_APP_TAG`** and **`docker compose up -d --no-deps --force-recreate $MPC_AUTH_NODE_APP_COMPOSE_SERVICE`** (default **`dashboard`**) so **continuumdao-node-app** tracks new registry layers when mpc-auth is updated.
