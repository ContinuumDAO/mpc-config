...
- **Deterministic reinstall:** **`PublicMgtKey`** plus **`DeterministicNodeKey: true`** and **`bootstrap_key/ed25519_private.hex`** (matching seed) reproduce the same mpc-auth **`nodeKey`** on fresh Mongo. Auto-generation of that bundle is **`./process_config.sh`** via **`tools/bootstrap_key_provision.py`** when **`PublicMgtKey`** is empty, or provisioning syncs **`DeterministicNodeKey`** when **`PublicMgtKey`** is preset and **`bootstrap_key/`** is present (**`provision-node.sh` pattern 2**). **Back up `bootstrap_key/`** securely (and restore **`PublicMgtKey`/Mongo**) as documented.
