...
| Agree/Reject        | `POST /signRequestAgree`     | Fetch nonce from **`GET /getPublicMgtKeyNonce`**, build JSON with **`requestId`**, **`nonce`**, **`nodeKey`**, **`accept`**, optional **`thoughts`**, **`clientSig` cleared**; sign; send as **`clientSig`**. |
