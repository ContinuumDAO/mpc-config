...
The agent uses an ed25519 key to authenticate API interactions. **`AUTH_KEY_PATH`** must be a **directory** containing the private key file; the default basename is **`mpc_auth_ed25519`** (set **`AUTH_KEY_FILENAME`** to override). If **`AUTH_KEY_PATH`** is unset, the file **`~/.ssh/mpc_auth_ed25519`** is used.
...
**Nonce for Ed25519:** Many management `POST` bodies include a **`nonce`** (or **`Nonce`**) that must match the node’s replay counter for **that** signing key. For agents using Ed25519, obtain it with **`GET /getPublicMgtKeyNonce`** (and **`?publicKey=<64_hex>`** when the agent key was added via **`POST /addManagementKey`**). **`GET /getNodeMgtKeyNonce`** applies only to the Ethereum **`NodeMgtKey`** (MetaMask); it is the wrong endpoint for Ed25519 and may show **`nonce: 0`** even after many Ed25519-signed requests. See **[API_IMPLEMENTATION.md](./API_IMPLEMENTATION.md)** (`GET /getNodeMgtKeyNonce`, `GET /getPublicMgtKeyNonce`) and **[SKILL.md](../skill/SKILL.md)** § Management API nonce.
