...
5. **Submit and complete the lifecycle:** The **`POST /multiSignRequest`** body must come **only** from the repo helpers in step 2. Agents must **not** create that JSON from scratch, hand-edit helper output, or change fields after the helpers run—only add management signing (**`clientSig`** / **`signedMessage`** per the guides). Then **`POST /multiSignRequest`** → peers **`POST /signRequestAgree`** → when ready, **EVM for AI agents** is **only** **`$MPA_PATH/scripts/executeSignResult.py`** (it **`POST`**s **`/triggerSignRequestById`** with **`txParams`** / **`messageHash`**, polls **`getSignResultById`**, broadcasts). Do **not** call **`POST /triggerSignRequestById`** or broadcast **directly** from the agent. Then **`POST /updateSignResultStatusById`**. **Human operators** may still use the web app or manual **`curl`** for trigger/execute (**`API_IMPLEMENTATION.md`** § **`POST /triggerSignRequestById`**). **EVM:** **`txParams`** and **`messageHash`** on trigger must match the signed preimage; otherwise **`GET /getSignRequestById?tx_params=1`** returns **`Data: null`** and Execute cannot rebuild the same tx. See **`API_IMPLEMENTATION.md`** and the node-app **`SIGN_REQUEST_TX_PARAMS.md`** (Get Sig parity).
