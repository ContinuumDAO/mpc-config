...
A **multiSignRequest** is a **formal proposal** stored on every node in the KeyGen. Supported automation builds **one or more EVM unsigned transactions** (calldata, chain id, nonces, fee fields) and uses **`./API_IMPLEMENTATION.md`** § **`POST /triggerSignRequestById`** with **`txParams`** / **`messageHash`** (or batch **`txParamsBatch`**). The proposal is **not** an MPC signature yet: peers **`POST /signRequestAgree`** until **threshold+1** accept. Then the originator runs the MPC signing step. **AI agents** use **only** **`$MPA_PATH/scripts/executeSignResult.py`**: it **`POST`s `/triggerSignRequestById`** with the right EVM fields, polls **`getSignResultById`**, and broadcasts. Do **not** call **`/triggerSignRequestById`** or broadcast **directly** from the agent except via this script (**`../skill/SKILL.md`**). **Human operators** may use the web app **Execute** tab or manual **`curl`** per **`API_IMPLEMENTATION.md`**.
