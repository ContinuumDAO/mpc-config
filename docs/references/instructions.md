...
A **multiSignRequest** is a **formal proposal** stored on every node in the KeyGen. Most proposals describe **one or more EVM unsigned transactions** (calldata, chain id, nonces, fee fields). **EIP-712** proposals (e.g. Uniswap **Permit2** **`PermitSingle`**) use a **32-byte typed-data digest** as **`msgHash`** and are **not** “gas-estimate a fake tx to Permit2 with JSON as `data`”—see **`./API_IMPLEMENTATION.md`** § **`POST /triggerSignRequestById`**. The proposal is **not** an MPC signature yet: peers **`POST /signRequestAgree`** until **threshold+1** accept. Then the originator runs the MPC signing step. **AI agents** use **only** **`$MPA_PATH/scripts/executeSignResult.py`**: for **RLP/unsigned-tx** requests it **`POST`s `/triggerSignRequestById` with `txParams` and `messageHash`**, polls **`getSignResultById`**, and broadcasts. For **digest-only** requests, trigger **omits** **`txParams`** and **`messageHash`** (see **`$MPA_PATH/scripts/mpc_sign_request_digest.py`**); **`executeSignResult.py`** then skips raw broadcast for Permit2. Do **not** call **`/triggerSignRequestById`** or broadcast **directly** from the agent except via this script (**`../skill/SKILL.md`**). **Human operators** may use the web app **Execute** tab or manual **`curl`** per **`API_IMPLEMENTATION.md`**.
...
2. **Build** the unsigned **`POST /multiSignRequest`** body **only** with a repo helper: **`$MPA_PATH/scripts/generateSignRequestWithFoundryScript.py`** (Foundry), **`$MPA_PATH/scripts/generateMultiSignRequestFromCompose.py`** (compose JSON), a script under **`$MPA_PATH/recipes/`**, or **`$MPA_PATH/scripts/multiSignJoin.py`** to merge **two** helper outputs—following **`./AI_AGENT_FORGE_SIGNREQUEST.md`**, **`./AI_AGENT_COMPOSE_MULTISIGNREQUEST.md`**, and **`../skill/SKILL.md`** (helpers-only rule).
3. **Review** **`Purpose`** (≤256 characters) and transaction fields (chain id, calldata, gas, nonces) against group intent and messages.
4. **Sign for HTTP:** add **management** **`clientSig`** (and any other fields **`./API_IMPLEMENTATION.md`** requires for your deployment). Ed25519 management signing, nonces, and tools: **`./ED25519_MANAGEMENT_KEY_SIGNING.md`**.
5. **Submit and complete the lifecycle:** The **`POST /multiSignRequest`** body must come **only** from the repo helpers in step 2. Agents must **not** create that JSON from scratch, hand-edit helper output, or change fields after the helpers run—only add management signing (**`clientSig`** / **`signedMessage`** per the guides). Helpers emit **`bodyForSign`** and **`messageToSign`**. Then **`POST /multiSignRequest`** → peers **`POST /signRequestAgree`** → when ready, use **`$MPA_PATH/scripts/executeSignResult.py`**: for **standard EVM unsigned-tx** sign requests it posts **`/triggerSignRequestById`** with **`txParams`** and **`messageHash`** (or batch **`txParamsBatch`**), then polls and broadcasts; for **EIP-712** (e.g. Permit2) it **omits** **`txParams`** and **`messageHash`** (see **`./API_IMPLEMENTATION.md`** and **`$MPA_PATH/scripts/mpc_sign_request_digest.py`**). Do **not** call **`/triggerSignRequestById`** or broadcast **directly** from the agent. Then **`POST /updateSignResultStatusById`** when you have on-chain results. **Human operators** may use the web app or manual **`curl`**. For **EVM** parity, use **`--sign-request-file`** if **GET** omits fields; see **`API_IMPLEMENTATION.md`** and **continuumdao-node-app** **`SIGN_REQUEST_TX_PARAMS.md`**.

**Further detail (this repo):** Payload shapes, CLI flags, and signing—**`./AI_AGENT_COMPOSE_MULTISIGNREQUEST.md`**, **`./AI_AGENT_FORGE_SIGNREQUEST.md`**, **`./API_IMPLEMENTATION.md`**, **`./ED25519_MANAGEMENT_KEY_SIGNING.md`**, **`./AI_AGENT_NEW_SESSION.md`**, **`../skill/SKILL.md`**. Node owner key setup: **`../CONFIGURING_ED25519_KEYS.md`**. Management API base URL: **`$MPC_AUTH_URL:$MANAGEMENT_PORT`** (host-only + `ManagementAPIsPort` in **`configs.yaml`**); load from **`$MPA_PATH/.env`** when present (**`../skill/SKILL.md`** **Environment**). Install Python dependencies as described in those guides.
...
(3) The AI Agent prepares **one** multiSignRequest proposal using **one** of the supported helper paths (never hand-built JSON). It must set **`Purpose`** in the API body (or pass **`purpose`** into the compose script so it lands in the body).

    - **Path A — Foundry:** Write or obtain a Foundry script; run **`forge script … --sender <MPC Ethereum address>`** so **`broadcast/.../run-latest.json`** exists; run **`$MPA_PATH/scripts/generateSignRequestWithFoundryScript.py`** on that file to build the **`POST /multiSignRequest`** body. **Guide:** `./AI_AGENT_FORGE_SIGNREQUEST.md`.
    - **Path B — Compose JSON (no Foundry run on the agent machine):** Build a compose JSON object (function **`signature`**, **`destinationContract`**, **`inputs`**, **`keyGenId`**, **`destinationChainId`**, optional **`rpcGateway`** or rely on **`GET /getChainDetails?chain_id=…`**); run **`$MPA_PATH/scripts/generateMultiSignRequestFromCompose.py`**; add management signing; **POST /multiSignRequest**. **Guide:** `./AI_AGENT_COMPOSE_MULTISIGNREQUEST.md`.
    - **Path C — Recipes:** Run a thin CLI under **`$MPA_PATH/recipes/`** (see **`./AI_AGENT_COMPOSE_MULTISIGNREQUEST.md`** recipes table). Same rule: output is helper-produced JSON only.
    - **Merging two helper outputs** into **one** batch: **`$MPA_PATH/scripts/multiSignJoin.py`** (same chain; **`--first-nonce`** = first tx nonce in the merge). **Chaining:** run **`multiSignJoin.py`** again on prior stdout plus another helper file.

(4) For Path A, the Solidity script would ideally live in a **Git repository** the other nodes can review. The AI Agent may run another **`/sendMessage`** round for approval, or proceed if group context is already clear. **`forge script <Contract>.s.sol:<Function> --rpc-url <RPC_URL> --sender <MPC address>`** produces **`run-latest.json`** under **`broadcast/`**; feed that path into **`$MPA_PATH/scripts/generateSignRequestWithFoundryScript.py`**.
...
(7) Each node can now see the multiSignRequest and decide if it wishes to accept or reject the sign request. Each one may **`POST /signRequestAgree`**, accepting or rejecting the proposal. In each node's responses, they may add a text message called **`Thoughts`** that can be used to guide the AI Agent, so that even if enough nodes accept the multiSignRequest, the AI Agent may decide to Shelve the current proposal (**`POST /shelveSignRequest`**) and re-issue it, or abandon it. If the multiSignRequest has at least **threshold+1** accepts (**`GET /isSignRequestReadyById`**) and the originator proceeds, run **only** **`$MPA_PATH/scripts/executeSignResult.py`** with **`--sign-request-id`** and, for **EVM** flows, **`--sign-request-file`** = saved helper stdout (so **`txNonce`**, gas, **`txParams`**, etc., exist when **GET** omits them). The script calls **`/triggerSignRequestById`**: for **unsigned EVM tx** preimages, with **`txParams`** / **`messageHash`** (or batch **`txParamsBatch`**); for **EIP-712 Permit2**, **without** those fields (**`mpc_sign_request_digest.py`** / **`./API_IMPLEMENTATION.md`**). Poll **`getSignResultById`**; broadcast only for **EVM** chain txs. **Human operators** may use the web app or **`curl`**. After successful on-chain steps, **`POST /updateSignResultStatusById`**.
...
The agent is run in an environment with the following variables configured. **Canonical list:** **`../skill/SKILL.md`** **Environment** and **`./AI_AGENT_NEW_SESSION.md`** (bootstrap: **`$MPA_PATH/.env`**, symlinks, **`GET /health`**, **`KEYGEN_ID`**).
...
The agent uses an ed25519 key to authenticate API interactions. This key is stored somewhere on the node (local to the agent). **`AUTH_KEY_PATH`** must be a **directory**; the private key file inside it defaults to **`mpc_auth_ed25519`** (override with **`AUTH_KEY_FILENAME`**). If **`AUTH_KEY_PATH`** is unset, the resolved file path is **`~/.ssh/mpc_auth_ed25519`**. See **`scripts/mpc_mgt_helpers.py`** (`resolve_ed25519_private_key_file`). Management signing details: **`./ED25519_MANAGEMENT_KEY_SIGNING.md`**.

### MPA_PATH, MPC_AUTH_URL, MANAGEMENT_PORT, MPC_CONFIG_PATH

Load **`$MPA_PATH/.env`** when present (**`../skill/SKILL.md`**). **`MPC_AUTH_URL`** (host-only) and **`MANAGEMENT_PORT`** form the management API base URL **`$MPC_AUTH_URL:$MANAGEMENT_PORT`**. **`MPA_PATH`** points at scripts, **`references/`**, **`recipes/`**, **`tools/`**. **`MPC_CONFIG_PATH`** is the **mpc-config** repo root (**`configs.yaml`**, **`docs/`**). Node owner bootstrap of **`PublicMgtKey`**: **`../CONFIGURING_ED25519_KEYS.md`**.
