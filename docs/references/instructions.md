...
A **multiSignRequest** is a **formal proposal** stored on every node in the KeyGen: it describes **one or more EVM transactions** the shared MPC wallet would execute (calldata, chain id, nonces, fee fields, etc.). It is **not** an MPC signature yet—peers **`POST /signRequestAgree`** until **threshold+1** accept. After that, an originator runs the MPC signing step and execution. **EVM (AI agents):** use **only** **`$MPA_PATH/scripts/executeSignResult.py`** to **`POST /triggerSignRequestById`** (with **`txParams`** / **`messageHash`**), poll **`getSignResultById`**, and broadcast—do **not** call **`POST /triggerSignRequestById`** or broadcast **directly** from the agent (**`../skill/SKILL.md`**). **Human operators** may use the web app **Execute** tab or manual **`curl`** per **`API_IMPLEMENTATION.md`**.
...
5. **Submit and complete the lifecycle:** The **`POST /multiSignRequest`** body must come **only** from the repo helpers in step 2. Agents must **not** create that JSON from scratch, hand-edit helper output, or change fields after the helpers run—only add management signing (**`clientSig`** / **`signedMessage`** per the guides). Helpers emit **`bodyForSign`** and **`messageToSign`** (same envelope for compose, Foundry, recipes, and **`multiSignJoin`**). Then **`POST /multiSignRequest`** → peers **`POST /signRequestAgree`** → when ready, **EVM for AI agents** is **only** **`$MPA_PATH/scripts/executeSignResult.py`** (it **`POST`**s **`/triggerSignRequestById`** with **`txParams`** or **`txParamsBatch`** and **`messageHash`**, polls **`getSignResultById`**, broadcasts). Do **not** call **`POST /triggerSignRequestById`** or broadcast **directly** from the agent. Then **`POST /updateSignResultStatusById`**. **Human operators** may still use the web app or manual **`curl`** for trigger/execute (**`API_IMPLEMENTATION.md`** § **`POST /triggerSignRequestById`**). **EVM:** trigger **`txParams`** / **`messageHash`** (or batch **`txParamsBatch`**) must match the signed preimage; otherwise **`GET /getSignRequestById?tx_params=1`** may omit usable fields and Execute cannot rebuild the same tx. See **`API_IMPLEMENTATION.md`** and the node-app **`SIGN_REQUEST_TX_PARAMS.md`** (Get Sig parity).
...
(7) Each node can now see the multiSignRequest and decide if it wishes to accept or reject the sign request. Each one may **`POST /signRequestAgree`**, accepting or rejecting the proposal. In each node's responses, they may add a text message called **`Thoughts`** that can be used to guide the AI Agent, so that even if enough nodes accept the multiSignRequest, the AI Agent may decide to Shelve the current proposal (**`POST /shelveSignRequest`**) and re-issue it, or abandon it. If the multiSignRequest has at least **threshold+1** accepts (**`GET /isSignRequestReadyById`**) and the originator proceeds, **EVM (AI agent):** run **only** **`$MPA_PATH/scripts/executeSignResult.py`** with **`--sign-request-id`** and **`--sign-request-file`** pointing to the **saved helper stdout** from the same run (**compose, Foundry, recipe, or `multiSignJoin`**—so **`bodyForSign`** with **`txNonce`**, gas, **`txParams`** / **`proposalTxParams`**, etc., is available when **GET** omits them). The script **`POST`**s **`/triggerSignRequestById`** with **`txParams`** or **`txParamsBatch`** and **`messageHash`**, polls **`getSignResultById`**, and broadcasts—do **not** call **`POST /triggerSignRequestById`** or broadcast **directly** from the agent (**`../skill/SKILL.md`**). **Human operators** may use the web app or **`curl`** per **`API_IMPLEMENTATION.md`**. **`POST /triggerSignRequestById`** must include the correct proposal/trigger params for EVM so execution matches the agreed preimage; omitting them breaks Execute. After execution, **`POST /updateSignResultStatusById`** with **`executed`** and **`transactionHash`** (or batch hashes).
