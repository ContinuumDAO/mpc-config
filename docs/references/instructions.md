...
(3) The AI Agent then can either generate a forge script using generateSignRequestWithFoundryScript.py, or it can generate a multiSignRequest directly using generateMultiSignRequestFromCompose.py for single or batched transactions. The AI Agent should also include the Purpose of the multiSignRequest in that dedicated string field.

(4) For the Foundry script route, the script would ideally be pushed to a github repository that is accessible by the nodes in the Group. The AI Agent can either look for a go-ahead from the other nodes with another /sendMessage round, or if it has a clear idea what to do, it can proceed to use the 'forge script' command on a script (forge script <Something>.s.sol:<Something> --rpc-url <RPC_URL> --sender <the MPC public address>) which creates a file called run-latest.json in the folder broadcast, that can be fed into the included Python script scripts/generateSignRequestWithFoundryScript.py

(5) For the direct multiSignRequest route using generateMultiSignRequestFromCompose.py the AI Agent should populate a JSON input for the script including one or more function calls and choosing whether to use an existing chain config using /getChainDetails?chain_id=<the chainID> for the RPC URL and gas parameters, or to use the user directed values and to estimate the gas itself. The output from the script can be posted to POST /multiSignRequest afterwards for other nodes to agree.

(6) Similarly for the output from generateSignRequestWithFoundryScript.py which can then be used to create a POST /multiSignRequest. In both cases, this will be visible to all the other nodes via the normal MPA wallet communications system. If there are more than one proposed transactions, then this is called a batch sign request. The /multiSignRequest includes a text string called 'Purpose' (max 256 characters) that should summarize what the trasactions are designed to do.
