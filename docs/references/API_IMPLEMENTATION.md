...
**Automation note (this repo):** **continuum-node-sdk** MCP tools (**`trigger_sign_result`**, **`broadcast_sign_result`**) and **continuumdao-node-app** assume **`POST /multiSignRequest`** carries **unsigned EVM transaction** material (**`msgRaw`** / **`messageRawBatch`** + **`txParams`** alignment). **`POST /triggerSignRequestById`** therefore includes **`txParams`** / **`txParamsBatch`** and **`messageHash`** (and for batch, optionally **`messageHashes`** / **`messageRawBatch`**) as below. mpc-auth may still accept a **minimal** trigger body (management **`requestId`**, **`nonce`**, **`sig`** only) for **legacy** proposals that are not standard unsigned txs; that path is **not** used by the supported harness automation.
