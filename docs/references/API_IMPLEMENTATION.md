...
| File | Content | Mode |
|------|---------|------|
| `/app/added_keys/added_key_<N>` | PKCS#8 PEM private key | `0600` |
| `/app/added_keys/added_key_<N>.pub` | 64-hex public key + newline | `0644` |

`<N>` is the **Added key N** slot (1-based index among Mongo **`ExtraPublicMgtKeys`** rows, including soft-removed slots). In **Docker** (mpc-config compose), **`app`** and **`continuum-mcp`** bind-mount host **`./added_keys`** and **`./bootstrap_key`** at **`/app/added_keys`** and **`/app/bootstrap_key`** (bootstrap read-only on continuum-mcp). Both services use **`WORKDIR /app`**; key paths are always under **`/app`**.
...
Also deletes **`/app/added_keys/added_key_<N>`** and **`added_key_<N>.pub`** when present (same `<N>` as the slot; no error if files are already missing).
...
| **Provisioning** | **`process_config.sh`** sets **`AgentLlmConfigDir`**, creates runtime **`./agent_llm_config/`** (gitignored on nodes; seeded from **`agent_llm_config.defaults/`** once per file if missing), including **`Skills/`**, **`trade-desk.yaml`**, **`cron/trade-cron.yaml`**, **`cron/`**, **`cron/runs/`**, **`conversations/`** at runtime), and sets compose env by default; use **`--no-agent-llm-config-path`** or **`PROCESS_CONFIG_SKIP_AGENT_LLM_CONFIG_PATH=1`** to skip |
...
| **`agent_llm_config/MCP_default_servers.json`** | Legacy bootstrap file for **continuum**, **coinmarketcap-public**, and **coinbase-public** (seeded once by **`process_config.sh`**); used when mpc-auth first migrates/seeds the database. |

Each active server entry: `id`, `displayName`, optional **`kind`** (`search-engine` or `research-data` — catalog metadata so mpc-auth classifies the server without a binary rebuild), optional **`searchTags`** (host catalog search aliases; `id` and `displayName` are always indexed), optional **`setupUrl`** (catalog/UI only — setup documentation link for the **Name** column in the node app; not used at MCP connect time), **`initialLoad`** (connect at chat startup so tools are available in a new agent window), **`aiReady`** (include id in plan-mode orchestration prompts for `mpc-orchestrate` manifests; **continuum** is always ai-ready), optional `apiKey` (HTTP auth, stored in database; redacted in list responses), optional `apiKeyEnvVar` (HTTP auth loaded from agent environment variables), optional **`apiKeyHeader`** (HTTP header name when using `apiKeyEnvVar`; default Bearer), optional `envVars` (STDIO: inject named variables into the child process), optional **`useUserFolder`** (STDIO: set **`HOME`** to **`/app/user_folder`** for persistent artefacts), optional **`runtime`** (STDIO: install/check before connect or on add/activate). Adding a catalog MCP is an **mpc-config** change (`MCP_servers.json`); do not add per-server names to mpc-auth.
...
**HTTP** (`transport`: `http`, default): `url` — Streamable HTTP MCP endpoint; empty `url` on default **continuum** uses compose **continuum-mcp** URL (`/mcp`). Auth: inline `apiKey` and/or `apiKeyEnvVar` (set the value via **`POST /addEnvironmentVariable`**). Catalog **coinmarketcap-public** (same **continuum-mcp** image, path **`/mcp/cmc-public`**): `http://continuum-mcp:8446/mcp/cmc-public`; wraps the [CoinMarketCap Keyless Public API](https://pro.coinmarketcap.com/api/documentation/pro-api-reference/keyless-public-api) (`get_kline_candles`, `get_global_metrics_latest`, `get_fear_and_greed_latest`, `get_fear_and_greed_historical`, `get_cmc100_latest`, `get_altcoin_season_index_latest`, `search_dex_tokens`, `get_dex_token`, `get_dex_token_pools`, `get_dex_pair_quotes`, `get_simple_price`, `get_crypto_quotes_latest`, `get_crypto_ohlcv_historical`); keyless tools need no API key; Pro OHLCV needs **`COINMARKETCAP_API_KEY`** on continuum-mcp; also seeded in **`MCP_default_servers.json`** with **`initialLoad`**: false — load via **`agent_load_mcp_server`** when needed (e.g. chart OHLCV). Catalog **coinbase-public** (same **continuum-mcp** image, path **`/mcp/coinbase-public`**): `http://continuum-mcp:8446/mcp/coinbase-public`; Coinbase Advanced Trade public market data (`get_product_candles`, `list_products`, `search_products`, `get_product_ticker`, `get_product_book`); keyless by default; optional Variables **`COINBASE_CDP_API_KEY_NAME`** + **`COINBASE_CDP_API_PRIVATE_KEY`** (ECDSA PEM) unlock authenticated brokerage routes without blocking public tools; seeded in **`MCP_default_servers.json`** with **`initialLoad`**: false. Catalog **vpn** (same **continuum-mcp** image, path **`/mcp/vpn`**): `http://continuum-mcp:8446/mcp/vpn`; admin WireGuard + peer egress MCP tools (`get_vpn_status`, `set_vpn_enabled`, `download_vpn_admin_client_config`, egress sharing/revoke/download); no API key; defaults **`initialLoad`**: false and **`aiReady`**: false. Catalog **technical-indicators** (path **`/mcp/ta`**): `http://continuum-mcp:8446/mcp/ta`; wraps [fast-technical-indicators](https://www.npmjs.com/package/fast-technical-indicators) (`list_technical_indicators`, `calculate_technical_indicator`); no API key; defaults **`initialLoad`**: false and **`aiReady`**: false (opt in via Node → AI Agent → MCP servers). Default **etherscan** ([official Etherscan MCP](https://docs.etherscan.io/build-with-ai/mcp)) uses `https://mcp.etherscan.io/mcp` with `apiKeyEnvVar`: `ETHERSCAN_API_KEY` (default Authorization Bearer; [create API key](https://docs.etherscan.io/set-up-your-api-key)). This is the only official Etherscan data MCP — marketplace or stdio listings that call themselves “Etherscan MCP” are unofficial and unaffiliated. Streamable HTTP; pass `chainid` per call (one key covers 60+ chains). 20 read-only tools: `get_native_balance`, `get_token_balances`, `get_transactions`, `get_internal_transactions`, `get_transaction_by_hash`, `get_transaction_receipt`, `get_transaction_status`, `get_transaction_receipt_status`, `get_token_transfers`, `get_token_info`, `get_token_top_holders`, `get_contract_source`, `get_contract_abi`, `get_contract_creation`, `get_logs`, `get_address_labels` (Pro), `get_funded_by`, `get_supported_chains`, `get_gas_oracle`, `get_block_by_timestamp`. Each tool call consumes Etherscan API quota. Do not confuse with the [Docs MCP](https://docs.etherscan.io/build-with-ai/docs-mcp) at `https://docs.etherscan.io/mcp` (no-key documentation search only). Default **blockscout** ([official Blockscout MCP](https://mcp.blockscout.com/)) uses `https://mcp.blockscout.com/mcp` with `apiKeyEnvVar`: `BLOCKSCOUT_PRO_API_KEY` and `apiKeyHeader`: `Blockscout-MCP-Pro-Api-Key` ([PRO API key](https://dev.blockscout.com); free tier, no card). Multi-chain Blockscout PRO gateway — use `get_chains_list` to resolve eligible chains; prefer over Etherscan MCP for `*.blockscout.com` explorers. Default **dune** ([Dune MCP](https://docs.dune.com/api-reference/agents/mcp)) uses `https://api.dune.com/mcp/v1` with `apiKeyEnvVar`: `DUNE_API_KEY` and `apiKeyHeader`: `x-dune-api-key` (API key from [dune.com/settings/api](https://dune.com/settings/api)). Default **gdelt-cloud** ([GDELT Cloud MCP](https://gdeltcloud.com/product/mcp), [quickstart](https://docs.gdeltcloud.com/mcp/quickstart)) uses `https://gdelt-cloud-mcp.fastmcp.app/mcp` with `apiKeyEnvVar`: `GDELT_API_KEY` (default Authorization Bearer; key `gdelt_sk_…` from [gdeltcloud.com/api-keys](https://gdeltcloud.com/api-keys)). Progressive discovery wrappers (`gdelt_cloud_tool_list` / `gdelt_cloud_tool_get` / `gdelt_cloud_tool_call`) plus Energy Data, macro finance, prediction markets, and web research. Requires a GDELT Cloud plan with API/MCP access. Catalog **business-latest** (same **continuum-mcp** image, path **`/mcp/business-latest`**): `http://continuum-mcp:8446/mcp/business-latest`; free RSS (BBC Business, CNBC Business, MarketWatch, Forbes Business, Reuters World via Google News, RT Business); tools `list_business_sources`, `get_business_latest`, `search_business_latest`; no API key; defaults **`initialLoad`**: false. Catalog **world-affairs** (same **continuum-mcp** image, path **`/mcp/world-affairs`**): `http://continuum-mcp:8446/mcp/world-affairs`; free RSS (BBC World, Al Jazeera, The Guardian World, DW, France 24, NPR, CNN World, RT News); tools `list_world_affairs_sources`, `get_world_affairs_latest`, `search_world_affairs_latest`; Guardian / NPR / CNN / RT carry `biasNote`; no API key; defaults **`initialLoad`**: false. Catalog **crypto-latest** (same **continuum-mcp** image, path **`/mcp/crypto-latest`**): `http://continuum-mcp:8446/mcp/crypto-latest`; free RSS (CoinDesk, The Block, Cointelegraph, Decrypt, Blockworks, The Defiant, Bitcoin Magazine, Crypto Potato, CryptoSlate, Good Morning Crypto, OpenZeppelin); tools `list_crypto_sources`, `get_crypto_latest`, `search_crypto_latest`; no API key; defaults **`initialLoad`**: false. Not CryptoPanic. Add more feeds in continuum-node-sdk `src/core/crypto-latest/sources.ts`. Catalog **crypto-security** (same **continuum-mcp** image, path **`/mcp/crypto-security`**): `http://continuum-mcp:8446/mcp/crypto-security`; free security RSS (QuillAudits, SlowMist, Immunefi, BlockSec, CertiK, Rekt, Trail of Bits); tools `list_crypto_security_sources`, `get_crypto_security_latest`, `search_crypto_security`; no API key; defaults **`initialLoad`**: false. Catalog **coin-bureau-newsletters** (same **continuum-mcp** image, path **`/mcp/coin-bureau-newsletters`**): `http://continuum-mcp:8446/mcp/coin-bureau-newsletters`; Coin Bureau weekly newsletter archive (no public RSS — archive HTML + `server-sitemap-newsletters.xml`); tools `get_coin_bureau_latest`, `search_coin_bureau_newsletters`; no API key; defaults **`initialLoad`**: false. Catalog **crypto-banter-newsletters** (same **continuum-mcp** image, path **`/mcp/crypto-banter-newsletters`**): `http://continuum-mcp:8446/mcp/crypto-banter-newsletters`; Crypto Banter Substack RSS (The Insider, Good Morning Crypto, The Daily Candle); tools `list_crypto_banter_sources`, `get_crypto_banter_latest`, `search_crypto_banter_newsletters`; no API key; defaults **`initialLoad`**: false. Catalog **arkham-intel** (path **`/mcp/arkham-intel`**): `http://continuum-mcp:8446/mcp/arkham-intel`; wraps the official [Arkham Intel API](https://arkm.com/api/docs) at `https://api.arkm.com` (no hosted Arkham MCP); tools `list_arkham_api_paths`, `arkham_api_request`; Variable **`ARKHAM_API_KEY`** (`apiKeyEnvVar`), header **`API-Key`**; request access at [arkm.com/api](https://arkm.com/api); defaults **`initialLoad`**: false. Default **exa** ([Exa MCP](https://exa.ai/docs/reference/exa-mcp), [Search API](https://exa.ai/docs/reference/search)) uses `https://mcp.exa.ai/mcp` with no API key (hosted free-plan rate limits). Do **not** embed `?exaApiKey=` in the catalog URL. For higher limits, set Variable **`EXA_API_KEY`** and `apiKeyHeader`: `x-api-key` on the active server (`apiKeyEnvVar` is omitted from the catalog so load is not blocked when unset). Tools `web_search_exa` and `web_fetch_exa`. After it is AI Ready, set **`AGENT_DEFAULT_SEARCH_MCP=exa`** for default search. Default **tavily** ([Tavily](https://www.tavily.com/), [MCP docs](https://docs.tavily.com/documentation/mcp)) uses `https://mcp.tavily.com/mcp` with `apiKeyEnvVar`: `TAVILY_API_KEY` (default Authorization Bearer; [get API key](https://www.tavily.com/)); do **not** embed `?tavilyApiKey=` in the catalog URL. Tools `tavily-search`, `tavily-extract`, `tavily-map`, `tavily-crawl`. After it is AI Ready, set **`AGENT_DEFAULT_SEARCH_MCP=tavily`** for default search. Default **kagi** ([official Kagi MCP](https://github.com/kagisearch/kagimcp), [API](https://help.kagi.com/kagi/api/overview.html)) uses `https://mcp.kagi.com/mcp` with `apiKeyEnvVar`: `KAGI_API_KEY` (default Authorization Bearer; [get API key](https://kagi.com/api/keys)). API usage is billed separately from a Kagi subscription. Tools `kagi_search_fetch` and `kagi_extract`. After it is AI Ready, set **`AGENT_DEFAULT_SEARCH_MCP=kagi`** for default search. Default **serpapi** ([official SerpApi MCP](https://serpapi.com/integrations/mcp), [GitHub](https://github.com/serpapi/serpapi-mcp)) uses `https://mcp.serpapi.com/mcp` with `apiKeyEnvVar`: `SERPAPI_API_KEY` (default Authorization Bearer; [get API key](https://serpapi.com/manage-api-key)); do **not** embed the key in the catalog URL. Tools `search` (Google / Bing / Yahoo / DuckDuckGo / YouTube / eBay / …). After it is AI Ready, set **`AGENT_DEFAULT_SEARCH_MCP=serpapi`** for default search. Default **perplexity** ([official Perplexity MCP](https://docs.perplexity.ai/docs/getting-started/integrations/mcp-server), [GitHub](https://github.com/perplexityai/modelcontextprotocol)) uses `https://api.perplexity.ai/mcp` with `apiKeyEnvVar`: `PERPLEXITY_API_KEY` (default Authorization Bearer; [get API key](https://console.perplexity.ai)). A Perplexity Pro subscription is not this key. Tools `perplexity_search`, `perplexity_ask`, `perplexity_research`, `perplexity_reason`. After it is AI Ready, set **`AGENT_DEFAULT_SEARCH_MCP=perplexity`** for default search. Default **coingecko** ([public MCP](https://mcp.api.coingecko.com/)) uses `https://mcp.api.coingecko.com/mcp` with no API key (shared rate limits). Default **coingecko-pro** ([Pro MCP](https://mcp.pro-api.coingecko.com/)) uses `https://mcp.pro-api.coingecko.com/mcp` with `apiKeyEnvVar`: `COINGECKO_API_KEY` and `apiKeyHeader`: `x-cg-pro-api-key` ([CoinGecko Pro auth](https://docs.coingecko.com/reference/authentication)). Default **equibles** ([Equibles hosted MCP](https://equibles.com/mcp), [open-source stack](https://github.com/daniel3303/Equibles)) uses `https://mcp.equibles.com/mcp` with `apiKeyEnvVar`: `EQUIBLES_API_KEY` (omit `apiKeyHeader` — default Bearer). Create an `eq_…` key at [equibles.com/dashboard/apikeys](https://equibles.com/dashboard/apikeys); free tier is 100 requests/day shared with the REST API (no card). Do **not** embed `?api_key=` in the catalog URL. `GetStockPrices` returns daily OHLCV (markdown table or `{ date, open, high, low, close, volume }` rows); `GetLatestPrices` is a latest-close snapshot. Default **financial-modeling-prep** ([official FMP MCP](https://site.financialmodelingprep.com/developer/docs/mcp-server)) uses `https://financialmodelingprep.com/mcp` with `apiKeyEnvVar`: `FMP_API_KEY` and `apiKeyHeader`: `apikey` ([get API key](https://site.financialmodelingprep.com/register)); do **not** embed `?apikey=` in the catalog URL (mpc-auth HTTP auth is header/Bearer-only). FMP historical / chart tools return `{ date, open, high, low, close, volume }` rows (envelopes `{ symbol, historical }` or `{ data: […] }`); continuum-node-sdk maps `date` to chart time. Default **coinmarketcap** ([CMC MCP](https://coinmarketcap.com/api/documentation/ai-agent-hub/mcp)) uses `https://mcp.coinmarketcap.com/mcp` with `apiKeyEnvVar`: `COINMARKETCAP_API_KEY` and `apiKeyHeader`: `X-CMC-MCP-API-KEY` ([get API key](https://pro.coinmarketcap.com/login)); 12 tools (quotes, TA, news, global metrics, narratives, on-chain metrics). Default **altfins** ([altFINS MCP](https://altfins.com/crypto-market-and-analytical-data-api/documentation/mcp-server)) uses `https://mcp.altfins.com/mcp` with `apiKeyEnvVar`: `ALTFINS_API_KEY` and `apiKeyHeader`: `X-Api-Key` ([create API key](https://altfins.com/crypto-market-and-analytical-data-api/documentation/how-to-create-an-altfins-api-key)). Default **coinglass** ([official CoinGlass MCP](https://docs.coinglass.com/reference/mcp-service), beta) uses `https://api-mcp.coinglass.com/mcp` with `apiKeyEnvVar`: `COINGLASS_API_KEY` and `apiKeyHeader`: `CG-API-KEY` ([API key](https://www.coinglass.com/account)). Same Variable as catalog **etf-flow**. Do **not** use default Bearer — the hosted server looks for **`CG-API-KEY`**. 30+ tools (funding, open interest, liquidations, order book, sentiment, ETF including `get_bitcoin_etf_flow_history`). MCP calls consume the CoinGlass API quota. Prefer this official HTTP row over community CoinGlass stdio wrappers (not in the catalog). Tool schemas may change while the service is in beta. Not an OHLCV chart source. Catalog **nansen** ([official Nansen MCP](https://docs.nansen.ai/mcp/overview)) uses `https://mcp.nansen.ai/ra/mcp` with `apiKeyEnvVar`: `NANSEN_API_KEY` and `apiKeyHeader`: `NANSEN-API-KEY` ([API key](https://app.nansen.ai/auth/agent-setup)). Do **not** use default Bearer — the hosted server looks for **`NANSEN-API-KEY`**. `tools/list` works without a key; tool calls consume Nansen API credits (`token_info`, `token_ohlcv`, smart-money balances, `address_portfolio`, `wallet_pnl_summary`, `general_search`, …). defaults **`initialLoad`**: false. Catalog **koinju** ([official Koinju MCP](https://docs.koinju.io/mcp-server)) uses `https://mcp.koinju.io/mcp` with `apiKeyEnvVar`: `KOINJU_API_KEY` and `apiKeyHeader`: `x-api-key` ([API key](https://koinju.io/pricing)). Do **not** use default Bearer. `find_markets` is keyless; `get_ohlcv` is the historical candle tool (chartable — keep `start`) and shares the REST quota with `get_trades` / funding / options tools. defaults **`initialLoad`**: false. Catalog **cryptorank** ([official CryptoRank MCP](https://docs.cryptorank.io/mcp-server)) uses `https://api.cryptorank.io/mcp` with `apiKeyEnvVar`: `CRYPTORANK_API_KEY` (default Bearer — do **not** set `apiKeyHeader` to `X-Api-Key`; that header is REST `/v3` only). Requires a PRO (or higher) Public API plan; [API key](https://cryptorank.io/public-api/dashboard). No keyless MCP tier. Tools: `resolve_entity`, funding rounds, funds, tokenomics/unlocks, `get_historical_currency`, news, exchanges. defaults **`initialLoad`**: false. Optional **`apiKeyHeader`** on HTTP servers: custom header instead of `Authorization: Bearer`.

**STDIO** (`transport`: `stdio`): `command` (executable, e.g. `npx`) and optional `args` (e.g. `["-y", "duckduckgo-mcp-server"]`). Optional **`envVars`**: names of agent environment variables injected into the child process (all listed names must be set before load). Optional **`apiKeyEnvVar`** on stdio entries: inject one named variable when set (does not block load if unset). Default **firefox** ([firefox-mcp-server](https://github.com/JediLuke/firefox-mcp-server)): `npx -y firefox-mcp-server`; requires **`npx`** on PATH (`runtime.requireCommands`). No API key. Playwright Firefox (not the operator’s logged-in profile); run **`npx playwright install firefox`** once if launch fails with a missing executable. After **`firefox`** is active and AI Ready, set **`AGENT_DEFAULT_SEARCH_MCP=firefox`** for default search. Default **mullvad-browser** and **gecko** ([gecko-mcp](https://github.com/Frumane/gecko-mcp)): `npx -y gecko-mcp`; requires **`npx`** on PATH (`runtime.requireCommands`). No API key. Drives a live Gecko browser (Mullvad Browser, Floorp, Firefox, …). For Mullvad, launch **`mullvad-browser -marionette`** (or set Variable **`GECKO_MCP_BACKEND=marionette`**). After **`mullvad-browser`** is active and AI Ready, set **`AGENT_DEFAULT_SEARCH_MCP=mullvad-browser`** for default search (browser session / Mullvad Leta — not a search API). Default **brave-search** ([Brave Search MCP](https://brave.com/search/api/guides/use-with-claude-desktop-with-mcp/)): `npx -y @modelcontextprotocol/server-brave-search`; requires **`BRAVE_API_KEY`** via **`POST /addEnvironmentVariable`** ([Brave Search API](https://brave.com/search/api/)) and **`npx`** on PATH (`runtime.requireCommands`). Default **google-search** ([mcp-google-search](https://mcpservers.org/servers/adenot/mcp-google-search), [GitHub](https://github.com/adenot/mcp-google-search)): `npx -y @adenot/mcp-google-search`; tools `search` (Google Custom Search API) and `read_webpage`; requires **`GOOGLE_API_KEY`** and **`GOOGLE_SEARCH_ENGINE_ID`** (Programmable Search Engine `cx`) via **`POST /addEnvironmentVariable`** ([Custom Search API](https://console.cloud.google.com/apis/library/customsearch.googleapis.com), [create search engine](https://programmablesearchengine.google.com/controlpanel/create)) and **`npx`** on PATH (`runtime.requireCommands`). Catalog **etherscan-community** ([mcp-etherscan-server](https://github.com/dennisonbertram/mcp-etherscan-server), **not** the official `mcp.etherscan.io` server): stdio `npx -y mcp-etherscan-server`; REST API v2 wrapper. Requires **`ETHERSCAN_API_KEY`** and **`npx`** on PATH (`runtime.requireCommands`). Etherscan states that unofficial “Etherscan MCP” listings are unaffiliated — do not treat this row as official, and prefer catalog **etherscan** (`https://mcp.etherscan.io/mcp`) when adding from repository. Default **finance-news** ([finance-news-mcp](https://github.com/jvenkatasandeep/finance-news-mcp), [LobeHub](https://lobehub.com/mcp/jvenkatasandeep-finance-news-mcp)): `uv run --with git+https://github.com/jvenkatasandeep/finance-news-mcp python -m main` (do not use `fastmcp run` with an `https://` URL — FastMCP treats that as a remote HTTP MCP endpoint, not a Python file); requires **`uv`** on PATH (`runtime.requireCommands`); public RSS feeds, no API key. Catalog **massive** ([official mcp_massive](https://massive.com/docs/ai-tools/quickstart)): `uvx --from git+https://github.com/massive-com/mcp_massive@v0.10.0 mcp_massive`; requires **`uv`** and Variable **`MASSIVE_API_KEY`**; no hosted HTTP MCP URL; tools `search_endpoints`, `call_api`, `query_data`. Default **binance** ([danishashko/binance-mcp-server](https://github.com/danishashko/binance-mcp-server)): `uv run` with FastMCP script from GitHub (public Spot API, no API key); tools prefixed `binance_` (e.g. `binance_get_klines`). Requires **`uv`** on PATH (`runtime.requireCommands`). Do not use `@snjyor/binance-mcp` — it logs to stdout and breaks stdio MCP initialize. Default **alpaca** ([official Alpaca MCP v2](https://github.com/alpacahq/alpaca-mcp-server)): `uvx alpaca-mcp-server@2`; requires **`ALPACA_API_KEY`** and **`ALPACA_SECRET_KEY`** via **`POST /addEnvironmentVariable`** ([Alpaca dashboard](https://app.alpaca.markets/paper/dashboard/overview)) and **`uv`** on PATH (`runtime.requireCommands`). Pin **`@2`** — v1 tool names are not compatible. Optional **`ALPACA_PAPER_TRADE`** (server default paper) and **`ALPACA_TOOLSETS`** are not required to load. OHLCV tools `get_stock_bars` / `get_crypto_bars` / `get_option_bars` return `{ t, o, h, l, c, v }` bars (array or symbol-keyed map); continuum-node-sdk maps them for charts. Live ticks use **`alpaca.latestTrade`** (needs the same keys on continuum-mcp / node-app). Default **edgartools** ([EdgarTools MCP](https://www.edgartools.io/edgartools-mcp-for-sec-filings/), [setup](https://edgartools.readthedocs.io/en/stable/ai/mcp-setup/)): `uvx --from edgartools[ai] edgartools-mcp`; requires **`EDGAR_IDENTITY`** (name and email for the [SEC fair-access](https://www.sec.gov/os/webmaster-faq#developers) User-Agent — not an API key) via **`POST /addEnvironmentVariable`** and **`uv`** on PATH (`runtime.requireCommands`). 13 SEC filing / company-research tools (`edgar_company`, `edgar_search`, `edgar_trends`, `edgar_ownership`, `edgar_monitor`, …); MIT-licensed, no paywall, no OHLCV. Default **alphavantage** ([Alpha Vantage MCP](https://mcp.alphavantage.co/)): `uvx --from marketdata-mcp-server marketdata-mcp` (progressive discovery via `TOOL_LIST` / `TOOL_CALL`); requires free **`ALPHA_VANTAGE_API_KEY`** via **`POST /addEnvironmentVariable`** ([get API key](https://www.alphavantage.co/support/#api-key)) and **`uv`** on PATH (`runtime.requireCommands`). Remote HTTP `https://mcp.alphavantage.co/mcp?apikey=…` is not used in the catalog because mpc-auth HTTP auth is header/Bearer-only. Default **whale-tracker** ([whale-tracker-mcp](https://github.com/kukapay/whale-tracker-mcp)): `uv run` with FastMCP script from GitHub; tools `get_recent_transactions`, `get_transaction_details`; requires **`WHALE_ALERT_API_KEY`** via **`POST /addEnvironmentVariable`** ([Whale Alert](https://whale-alert.io/)) and **`uv`** on PATH (`runtime.requireCommands`). Default **etf-flow** ([etf-flow-mcp](https://github.com/kukapay/etf-flow-mcp)): `uvx --from git+https://github.com/kukapay/etf-flow-mcp --with mcp[cli]>=1.6.0,<2 --with httpx --with tabulate etf-flow-mcp`; tool `get_etf_flow` (`coin`: `BTC` or `ETH`); requires **`COINGLASS_API_KEY`** via **`POST /addEnvironmentVariable`** ([CoinGlass account / API key](https://www.coinglass.com/account), [docs](https://docs.coinglass.com/reference/getting-started-with-your-api)) and **`uv`** on PATH (`runtime.requireCommands`). Pin **`mcp<2`** — upstream `mcp[cli]>=1.6.0` resolves to MCP 2.x which dropped FastMCP. Extra **`httpx`** and **`tabulate`** are required (not declared by the package; pandas `to_markdown` needs tabulate). Load is blocked until the Variable is set — the process raises at import if the key is missing. Same Variable as official HTTP **coinglass**; prefer **coinglass** for funding / OI / liquidations / general CoinGlass. **etf-flow** is the narrow markdown ticker×date table. Not an OHLCV chart source. Catalog **cryptopanic** ([cryptopanic-mcp-server](https://github.com/kukapay/cryptopanic-mcp-server), **not** an official CryptoPanic MCP — CryptoPanic publishes REST only): `uv run` with FastMCP script from GitHub; tool `get_crypto_news` (`kind`: `news` or `media`); requires **`CRYPTOPANIC_API_KEY`** and **`CRYPTOPANIC_API_PLAN`** via **`POST /addEnvironmentVariable`** ([developer API](https://cryptopanic.com/developers/api/)) and **`uv`** on PATH (`runtime.requireCommands`). Plan is the path segment (`/api/{plan}/v2/posts/`); the wrapper default `developer` was discontinued — set the slug from the CryptoPanic dashboard. Pin **`mcp<2`**. Prefer keyless catalog **crypto-latest** when no paid CryptoPanic plan. Not an OHLCV chart source. Default **messari** ([Messari MCP SDK](https://docs.messari.io/mcp-server/overview)): `npx -y @messari/sdk-ts-mcp@latest --client=claude --tools=dynamic` (dynamic API discovery via `list_api_endpoints` / `invoke_api_endpoint`); requires **`MESSARI_SDK_API_KEY`** via **`POST /addEnvironmentVariable`** ([messari.io/account/api](https://messari.io/account/api)) and **`npx`** on PATH (`runtime.requireCommands`). Hosted HTTP `https://mcp.messari.io/mcp` ([hosted MCP](https://docs.messari.io/mcp-server/hosted), tools `ask_messari`, timeseries, deep research) uses OAuth and is not in the catalog. Example default **x** ([DataWhisker/x-mcp-server](https://github.com/DataWhisker/x-mcp-server)): OAuth 1.0a via `TWITTER_API_KEY`, `TWITTER_API_SECRET`, `TWITTER_ACCESS_TOKEN`, `TWITTER_ACCESS_SECRET` from the [X Developer Portal](https://developer.x.com/en/portal/dashboard). Media upload may additionally need OAuth 2.0 vars (`TWITTER_OAUTH2_ACCESS_TOKEN` or `TWITTER_CLIENT_ID` / `TWITTER_CLIENT_SECRET` / `TWITTER_OAUTH2_REFRESH_TOKEN`) — add those via **Variables** and extend `envVars` on a user server entry if needed.
...
**Response data:** `{ "activeServers": [...], "availableCatalog": [...], "defaultServers": [...], "userServers": [...], "servers": [...] }` — **`activeServers`** / **`servers`**: database entries with `initialLoad`, `aiReady`, `builtin`, masked `apiKey`, `source` (`default` \| `user`), `removable`, optional **`setupUrl`**. **`availableCatalog`**: rows from the bind-mounted repository file **`agent_llm_config.defaults/MCP_servers.json`** that are not yet active on this node (`source`: `catalog`); includes catalog fields such as **`setupUrl`**, `transport`, `command` / `url`, `envVars`, and `runtime`.
...
**Body:** `{ "id", "displayName", "transport?", "url?", "command?", "args?", "apiKey?", "apiKeyEnvVar?", "apiKeyHeader?", "envVars?", "useUserFolder?", "runtime?", "setupUrl?", "initialLoad", "aiReady?", "nonce", "clientSig", "nodeKey" }` — upserts an **active** server in **`LocalAgentMcpServers`**; cannot replace builtin **continuum**. For HTTP auth without storing a secret in the database, set **`apiKeyEnvVar`** and the value via **`POST /addEnvironmentVariable`**. **`useUserFolder`** (STDIO only): sets **`HOME`** to **`/app/user_folder`**. Optional **`setupUrl`**: UI/documentation link only.
...
**Body:** `{ "id", "initialLoad?", "aiReady?", "nonce", "clientSig", "nodeKey" }` — copies the full catalog row from **`agent_llm_config.defaults/MCP_servers.json`** (including **`setupUrl`**, `transport`, `command` / `args` / `url`, `envVars`, `runtime`, etc.) into **`LocalAgentMcpServers`**. Fails if the id is already active or not in the catalog.
...
| **`agent-intent-rules`** | **`agent_llm_config/agent-intent-rules.yaml`** | **`agent_llm_config.defaults/agent-intent-rules.yaml`** | Free-text intent → pack boost + hints + optional **`always`** turn hints + **`loadMcpServers`** auto-load + optional **`verify`** (`interview` / `lookup` / `deliverable`) (never short-circuits the LLM). Protocol and third-party MCP policy lives here, not in mpc-auth. |
| **`continuum-dao-vote-policy`** | **`agent_llm_config/continuum-dao-vote-policy.yaml`** | **`agent_llm_config.defaults/continuum-dao-vote-policy.yaml`** | ContinuumDAO vote defaults (`trustedProposers`, `defaultAction`, type/signature/target/scam rules). Committee addresses are not operator-editable; agents fetch Constitution **Committee List**. Injected when the **`continuum-dao-vote-policy`** skill loads. Procedure prose stays in that skill. |
...
**Body:** `{ "kind", "content", "nonce", "clientSig", "nodeKey" }` — **`content`** is the full YAML file. Server validates structure per **`kind`** before atomic write. **`trade-desk`:** `version`, `universal`, `llmFallback`, `protocols`. **`orchestration-plan`:** plan schema. **`agent-intent-rules`:** rules schema. **`continuum-dao-vote-policy`:** `votePolicy.version`, `defaultAction` (`skip` / `nota` / `against` / `abstain`), optional `trustedAction` (`skip` / `nota` / `against` / `abstain` / `for`), 0x `trustedProposers` / `blockedProposers` / target addresses. Optional leftover `committeeProposers` is ignored; Committee membership is the Constitution **Committee List**. **`cron-trade`:** `version`, at least one of **`tradeConsensus`** / **`tradeBuild`**.
...
```json
{
  "mode": "trade|yield|research|portfolio|dao|hedging|custom",
...
**Automation note (this repo):** **continuum-node-sdk** MCP tools (**`trigger_sign_result`**, **`broadcast_sign_result`**) and **continuumdao-node-app** support two create shapes. **EVM txs:** `POST /multiSignRequest` carries unsigned transaction material (`msgRaw` / `messageRawBatch` + `proposalTxParams`); trigger includes `txParams` / `txParamsBatch` and refreshed `messageHash` / `messageHashes`. **EIP-712:** `extraJSON.signRequestKind` is `eip712` and `extraJSON.eip712` is a non-empty array of typed-data legs; trigger omits all `txParams*` and sends the stored digest (`messageHash` or `messageHashes`). mpc-auth may still accept a **minimal** trigger body (management **`requestId`**, **`nonce`**, **`sig`** only); that path is **not** used by the supported harness automation.
...
Minimal body above is structurally valid. **For supported automation,** add `messageHash` and `txParams` and/or `txParamsBatch` as below (see continuum-node-sdk **`trigger_sign_result`** / **`broadcast_sign_result`**).
