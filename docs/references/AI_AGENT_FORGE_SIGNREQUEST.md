...
5. **Submit and complete the lifecycle:** **`POST /multiSignRequest`** → peers agree → when ready, **AI agents** run **only** **`$MPA_PATH/scripts/executeSignResult.py`** for trigger + on-chain execution (see **`../skill/SKILL.md`**—do **not** call **`POST /triggerSignRequestById`** directly). The script builds **`/triggerSignRequestById`** with **`txParams`** and **`messageHash`** (or **`txParamsBatch`**). **`updateSignResultStatusById`** after successful broadcast. **`./instructions.md`** and **`./API_IMPLEMENTATION.md`** cover manual **`curl`** / web **Execute**.
...
1. **Canonical message to sign:** The helper’s **`messageToSign`** string — compact JSON of **`bodyForSign`** only (no **`clientSig`** / **`signedMessage`** in the signed bytes). It **must include a `purpose` key** (string, use `""` when there is no description) so the signed bytes match mpc-auth (see **`./API_IMPLEMENTATION.md`** § **`POST /multiSignRequest`**). Helpers should emit **`"purpose":""`** when the operator did not supply text.
...
3. **POST** the full body: all fields from **`bodyForSign`** plus **`clientSig`** plus **`signedMessage`**. **Recommended:** set **`signedMessage`** to the **same** string as **`messageToSign`**. If **`signedMessage`** is empty, mpc-auth may still verify **Ed25519** via **PublicMgtKey** using **canonical JSON** of the POST (with **`clientSig`** / **`signedMessage`** cleared), which always includes **`"purpose"`** — but agents should prefer the explicit **`messageToSign`** path for stability. Repo helpers set **`signedMessage`** automatically.
...
- **`triggerTxParams` / `triggerMessageHash`:** First index for trigger; batch proposals use **`proposalTxParams`** inside **`bodyForSign`**. Digest-only EIP-712 **multiSignRequest** payloads omit **`txParams`** / **`messageHash`** on trigger — see **`./API_IMPLEMENTATION.md`** and **`recipes/uniswapV4/README.md`**.
...
5. **Use** the returned `requestId` for:
   ...
   - **EVM unsigned tx (AI agent):** **`executeSignResult.py`** only — it runs **triggerSignRequestById** with **`txParams`** (or **`txParamsBatch`**) and **`messageHash`**, **`getSignResultById`**, and **`eth_sendRawTransaction`**, as in **API_IMPLEMENTATION.md** / **`../skill/SKILL.md`**. Save helper stdout as **`--sign-request-file`** when **GET** may omit proposal fields. For batch, the script consumes `data.batchSignatures[i]` like the web flow.
...
- [ ] Use the returned `requestId` for agree, then **`executeSignResult.py`** for trigger + signatures + broadcast. **`txParams`** / **`messageHash`** (or batch **`txParamsBatch`**) per **API_IMPLEMENTATION.md** / **`../skill/SKILL.md`**. **Recipes:** see **`$MPA_PATH/recipes/uniswapV4/README.md`** when not using this Forge path.
