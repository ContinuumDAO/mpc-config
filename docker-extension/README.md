...
1. Extension UI invokes **`host.cli.exec`** on the Windows host → **`wsl.exe`** → **`scripts/desktop-local-orchestrate.sh`** inside your WSL distro.
...
| Install orchestrator | Extension → `wsl.exe` → `desktop-local-orchestrate.sh` |
