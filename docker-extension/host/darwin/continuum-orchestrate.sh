...
Profiles:
  ...
  macos     install-node-macos-docker-desktop.sh (native macOS Docker Desktop)
...
Orchestrator options:
      --profile windows|linux|macos   Install profile (auto-detected when omitted)
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        --profile)
            ...
            case "$PROFILE" in
                windows|linux|macos) ;;
                *) die "--profile must be windows, linux, or macos (got: $PROFILE)" ;;
            esac
            shift 2
            ;;
        --repo-dir)
